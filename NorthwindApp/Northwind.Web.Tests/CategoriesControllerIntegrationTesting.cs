...
namespace Northwind.Web.Tests
{
    ...
    public class CategoriesControllerIntegrationTesting
    {
        ...
        [TestMethod]
        public async Task Create_DoesntCreateNewCategory_If_WrongDataInputed_And_ShowValidationErrors()
        {
            var client = GetTestHttpClient(() => NorthwindContextHelpers.GetInMemoryContext(true),
                new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
            var createForm = await client.GetStringAsync("/categories/create");
            var verificationToken = GetRequestVerificationToken(createForm);
            ...
            {
                [nameof(Category.CategoryName)] = String.Empty,
                [AspNetVerificationTokenName] = verificationToken,
            });

            var response = await client.PostAsync("/category/create", formContent);
        ...
        [TestMethod]
        public async Task Edit_ChangeExistingCategory_And_RedirectToList()
        {
            var context = NorthwindContextHelpers.GetInMemoryContext(true);
            var editingCategory = new CategoryGenerator(context).Generate();
            var category = new CategoryGenerator().Generate();
            context.SaveChanges();

            var client = GetTestHttpClient(() => NorthwindContextHelpers.GetInMemoryContext(),
                new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
            var createForm = await client.GetStringAsync($"/categories/edit/{editingCategory.CategoryId}");
            var verificationToken = GetRequestVerificationToken(createForm);

            var multipartContent = new MultipartFormDataContent();
            multipartContent.Add(new StringContent(editingCategory.CategoryId.ToString()),
                nameof(Category.CategoryId));
            multipartContent.Add(new StringContent(category.CategoryName), nameof(Category.CategoryName));
            multipartContent.Add(new StringContent(category.Description ?? String.Empty),
            ...
            var response = await client.PostAsync($"/categories/edit/{editingCategory.CategoryId}", multipartContent);
            context = NorthwindContextHelpers.GetInMemoryContext();
        ...
        [TestMethod]
        public async Task Edit_DoesntChangeCategory_If_WrongDataInputed_And_ShowValidationErrors()
        {
            var context = NorthwindContextHelpers.GetInMemoryContext(true);
            var editingCategory = new CategoryGenerator(context).Generate();
            context.SaveChanges();
            ...
            var formContent = new FormUrlEncodedContent(new Dictionary<string, string>()
            {
                [nameof(Category.CategoryId)] = editingCategory.CategoryId.ToString(),
                [nameof(Category.CategoryName)] = String.Empty,
                [AspNetVerificationTokenName] = verificationToken,
            });

            var response = await client.PostAsync($"/categories/edit/{editingCategory.CategoryId}", formContent);
            context = NorthwindContextHelpers.GetInMemoryContext();
            var category = context.Categories.First();
            var htmlSource = await response.Content.ReadAsStringAsync();

            var errors = GetValidationErrors(htmlSource);
            errors.Should().ContainMatch("*Название*");

            category.Should().BeEquivalentTo(editingCategory,
                options => options
        ...
        [TestMethod]
        public async Task Delete_DeleteCategory_And_RedirectToList()
        {
            var context = NorthwindContextHelpers.GetInMemoryContext(true);
            ...
            var client = GetTestHttpClient(() => NorthwindContextHelpers.GetInMemoryContext(),
                new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
            var createForm = await client.GetStringAsync($"/categories/delete/{category.CategoryId}");
            var verificationToken = GetRequestVerificationToken(createForm);
            ...
            var response = await client.PostAsync($"/categories/delete/{category.CategoryId}", formContent);
            context = NorthwindContextHelpers.GetInMemoryContext();
        ...
        [TestMethod]
        public async Task Delete_DoesntDeleteCategory_If_CategoryContainsProducts_And_ShowValidationErrors()
        {
            var context = NorthwindContextHelpers.GetInMemoryContext(true);
            var category = new CategoryGenerator(context)
                .WithProduct(new ProductGenerator())
                .Generate();
            context.SaveChanges();
            ...
            var response = await client.PostAsync($"/categories/delete/{category.CategoryId}", formContent);
            context = NorthwindContextHelpers.GetInMemoryContext();
            var storedCategory = await context.Categories.FindAsync(category.CategoryId) ?? new();
            var htmlSource = await response.Content.ReadAsStringAsync();
            var errors = GetValidationErrors(htmlSource);

            errors.Should().ContainMatch("*Нельзя удалять категории с привязанными товарами!*");

            storedCategory.Should().BeEquivalentTo(storedCategory,
                options => options
                .Including(c => c.CategoryId)
                .Including(c => c.CategoryName)
                .Including(c => c.Description)
        ...
        [TestMethod]
        public async Task Image_ReturnCategoryImage()
        {
            var context = NorthwindContextHelpers.GetInMemoryContext(true);
            var category = new CategoryGenerator(context)
                .WithProduct(new ProductGenerator())
            ...
            var response = await client.GetByteArrayAsync($"/categories/image/{category.CategoryId}");

            response.Should().BeEquivalentTo(category.Picture);
        }
        ...
        private static IEnumerable<string> GetValidationErrors(string htmlSource)
        {
            var context = BrowsingContext.New(Configuration.Default);
            var document = context.OpenAsync(req => req.Content(htmlSource)).Result;

            return document.QuerySelectorAll("[class*=validation]").Select(e => e.Text());
        }
    }
}
